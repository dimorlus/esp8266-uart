...
LOCAL STATUS ICACHE_FLASH_ATTR uart1_tx_one_char(uint8 TxChar)
{
    while (true)
	{
		...
		if ((fifo_cnt >> UART_TXFIFO_CNT_S & UART_TXFIFO_CNT) < 126) break;
	}
    ...
    while (READ_PERI_REG(UART_STATUS(UART0)) & (UART_RXFIFO_CNT << UART_RXFIFO_CNT_S))
     {
        ...
        *(pRxBuff->pWritePos) = RcvChar;
        ...
        if (uart_rx_cb) uart_rx_cb(RcvChar);
     }
}
...
{
  char *p1 = UartDev.rcv_buff.pWritePos;
  char *p2 = UartDev.rcv_buff.pReadPos;
  int avail = (p1 >= p2)?p1-p2:RX_BUFF_SIZE-(p2-p1);
  return avail;
}
...
{
    // rom use 74880 baut_rate, here reinitialize 8N1
...
void ICACHE_FLASH_ATTR clear_uart0(void)
 {
  while(uart_rx_avail()) uart_rx_one_char();
  UartDev.rcv_buff.pWritePos = UartDev.rcv_buff.pRcvMsgBuff;
  UartDev.rcv_buff.pReadPos  = UartDev.rcv_buff.pRcvMsgBuff;
 }
